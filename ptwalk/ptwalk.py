...
def sample(flogprob, ensemble, old_logprob_list, param):
    ...
    proposed_points=[propose_move(ensemble[i1], ensemble[i2], param) for (i1, i2) in zip(group1, group2)]
    ...
    for (i1, i2, up_prop, (yp, phi, k, beta)) in zip(group1, group2, log_prob_list, proposed_points):
        a=calc_a(ensemble[i2], ensemble[i1], old_logprob_list[i1], yp, up_prop, phi, k, beta)
        if uniform()<a:
            ensemble[i1]=yp
